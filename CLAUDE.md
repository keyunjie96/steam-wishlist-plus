...
**Tech Stack:** TypeScript, Lit 3.x, Chrome Extensions API (MV3), Jest
...
│   ├── options.ts          # Options page Lit component (<swp-options>)
│   ├── options.html        # Options page shell
│   ├── popup.ts            # Popup Lit component (<swp-popup>)
│   ├── popup.html          # Popup shell
│   ├── styles.css          # Extension styles
│   └── components/         # Shared Lit web components
│       ├── swp-theme.ts    # Shared CSS custom properties & theme
│       ├── swp-toggle.ts   # Toggle switch (mini & full variants)
│       ├── swp-stat-box.ts # Stat display card (compact & full)
│       ├── swp-status-message.ts # Toast success/error messages
│       ├── swp-section.ts  # Collapsible section with header
│       └── swp-icon-button.ts # Button with icon & loading state
